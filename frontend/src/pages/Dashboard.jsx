...
export default function Dashboard() {
  ...
  return (
  <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black p-6 text-gray-100">
    <div className="max-w-7xl mx-auto space-y-10">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-400 text-sm mt-2">Overview of your community</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map(({ label, value, icon: Icon, light, glow }) => (
          <div
            key={label}
            className={`rounded-2xl p-6 bg-gray-800/70 backdrop-blur-md border border-gray-700 shadow-lg transition hover:scale-105 hover:shadow-indigo-500/40`}
          >
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${light} shadow-md`}>
              <Icon className="w-6 h-6" />
            </div>
            <p className="text-2xl font-extrabold text-indigo-300">{value}</p>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {/* Finance Bar Chart */}
        <div className="rounded-2xl p-6 bg-gray-800/70 backdrop-blur-md border border-gray-700 shadow-lg">
          <h2 className="font-semibold text-pink-400 mb-4">Financial Overview</h2>
        ...
        <div className="rounded-2xl p-6 bg-gray-800/70 backdrop-blur-md border border-gray-700 shadow-lg">
          <h2 className="font-semibold text-pink-400 mb-4">Ticket Status</h2>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie
              ...
              >
                {ticketStatusData.map((_, i) => (
                  <Cell key={i} fill={PIE_COLORS[i]} stroke="black" strokeWidth={2} />
                ))}
              </Pie>
              <Tooltip
                contentStyle={{
      ...
      {/* Recent Tickets */}
      <div className="rounded-2xl shadow-lg bg-gray-800/70 backdrop-blur-md border border-gray-700 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-700 bg-gradient-to-r from-indigo-900 to-pink-900">
          <h2 className="font-semibold text-pink-400">Recent Maintenance Tickets</h2>
        </div>
        <div className="divide-y divide-gray-700">
          {tickets.slice(0,5).map(t => (
            <div key={t._id || t.id} className="px-6 py-4 flex items-center justify-between hover:bg-gray-700 transition">
              <div>
                <p className="text-sm font-medium text-gray-200">{t.title}</p>
                <p className="text-xs text-gray-400">Flat {t.flatNumber || t.flat_number} · {t.category || "Maintenance"}</p>
              </div>
              <span className={`text-xs px-3 py-1 rounded-full font-medium ${
                t.status === "Resolved" ? "bg-green-700 text-green-200" :
                t.status === "In Progress" ? "bg-yellow-700 text-yellow-200" :
          ...
          {tickets.length === 0 && (
            <p className="px-6 py-4 text-sm text-gray-500">No tickets yet.</p>
          )}
        </div>
      </div>
    </div>
